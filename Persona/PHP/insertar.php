    ...
    <div class="container">
      ...
      <div class="main-content">
          ...
          <div class="content">
			...
			$tipo_persona=mysql_escape_string($_POST['tipo_persona']);
      $departamento=mysql_escape_string($_POST['departamento']);     
			$sql = "INSERT INTO persona (dni, nombre, apellidos, email, tipo_persona) VALUES ('$dni', '$nombre', '$apellidos', '$email', '$tipo_persona')";

			if ($tipo_persona == "Alumno"){
        $sql2 = "INSERT INTO alumno (dni) VALUES ($dni)";
      }else{
        $sql2 = "INSERT INTO profesor (dni, departamento) VALUES ('$dni', '$departamento')";
      }    
			$result = mysql_query($sql);
			if($result === false) {
			    echo "No se pudieron insertar los datos de la persona. " . mysql_error() . ". " . mysql_errno();
			}
			else {
			}
      ...
      if ($tipo_persona == "Alumno"){
        if($result === false) {
            echo "No se pudo insertar la persona. " . mysql_error() . ". " . mysql_errno();
        }
        else {
              echo "<center><h1>La Persona se ha creado correctamente</h1></center>";
              echo "<center><a class=pure-button pure-button-primary href=/Facultad/Persona/PHP/consulta.php>Volver</a></center>"; 
        }        
      }else{
        if($result === false) {
            echo "No se pudo insertar la Persona. " . mysql_error() . ". " . mysql_errno();
