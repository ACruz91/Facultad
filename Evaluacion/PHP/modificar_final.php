      ...
      <div class="main-content">
          ...
          <div class="content">         
                <?php 
                  ...
                  $comentario_alumno=mysql_escape_string($_POST['comentario_alumno']);
                  $matricula=mysql_escape_string($_POST['matricula']); 
                  $id_asignatura=mysql_escape_string($_POST['id_asignatura']);
                  $nota_final=mysql_escape_string($_POST['nota_final']);
                  ...
                  //Consulta               
                  $sql = "UPDATE evaluacion SET comentario_alumno='$comentario_alumno', nota_final = $nota_final, matricula = '$matricula', id_asignatura = '$id_asignatura' WHERE id_asignatura = '$id_asignatura_antiguo' AND matricula = $matricula_antiguo";
                  $result = mysql_query($sql); 
                    if($result === false) {
                      echo "No se pudo modificar la Evaluacion. " . mysql_error() . ". " . mysql_errno();
                      echo "<center><a class=pure-button pure-button-primary href=/Facultad/Evaluacion/PHP/consulta.php>Volver</a></center>"; 
                    }
                    else {
                      echo "<center><h1>La Evaluacion se han guardado</h1></center>";
                      echo "<center><a class=pure-button pure-button-primary href=/Facultad/Evaluacion/PHP/consulta.php>Volver</a></center>"; 
                    } 
                ?> 
          </div>
